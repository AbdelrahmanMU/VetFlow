 ...
 * (purchasing ui.md states, STD-FE-031).
 */
@Component({
  selector: 'app-purchase-list-skeleton',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [VfSkeletonComponent],
  template: `
    <div class="skeleton-table" role="status" [attr.aria-label]="t.t('purchases.loading')">
...
export class PurchaseListSkeletonComponent {
  protected readonly t = inject(TranslationService);
  protected readonly rows = Array.from({ length: 8 }, (_, index) => index);
  protected readonly headerWidths = ['55%', '60%', '55%', '50%', '45%', '50%'];
  protected readonly cellWidths = ['60%', '70%', '55%', '45%', '55%', '50%'];
}

...
import { SalesReturnFailure } from './sales-return.models';

const FAILURE_MESSAGES: Readonly<Record<SalesReturnFailure, MessageKey>> = {
  invoiceNotCommitted: 'salesReturn.error.invoiceNotCommitted',
  exceedsReturnable: 'salesReturn.error.exceedsReturnable',
  lineComposition: 'salesReturn.error.lineComposition',
  notDraft: 'salesReturn.error.notDraft',
  noLines: 'salesReturn.error.noLines',
  traceUnusable: 'salesReturn.error.traceUnusable',
  belowZero: 'salesReturn.error.belowZero',
  conflict: 'salesReturn.error.conflict',
  notFound: 'salesReturn.error.notFound',
  unknown: 'salesReturn.error.unknown',
};
...
@Component({
  ...
  template: `
    <div class="page">
      ...
      @if (store.submit().kind === 'saved') {
        ...
        <div class="field">
          <label for="returnDate">{{ t.t('salesReturn.field.returnDate') }}</label>
            ...
            @for (line of store.lines(); track line.salesLineItemId) {
              <tr>
                ...
                <td>
                  ...
                  -->
                  <vf-number-input
                    [ngModel]="quantityOf(line.salesLineItemId)"
                    (ngModelChange)="setQuantity(line.salesLineItemId, $event)"
                    [min]="0"
                  />
                </td>
              </tr>
        ...
        <vf-textarea [label]="t.t('salesReturn.field.notes')" [(ngModel)]="notes" name="notes" />
        ...
        <div class="form-actions">
          <vf-button
            ...
            (pressed)="save()"
          >
            {{ saving() ? t.t('salesReturn.saving') : t.t('salesReturn.save') }}
          </vf-button>
  ...
  `,
  // Every class this template uses is defined here. The C5 screen this one mirrors defined none of
  // them, which live-browser verification caught: its banners and page chrome rendered as plain
  // unstyled text, because `.banner`/`.banner-success` exist nowhere in the codebase and the UI
  // Kit's `.vf-table` rules are ::ng-deep-scoped inside <vf-table> and never reach a raw table.
  // The tokens below are the real ones (--vf-success-soft / --vf-success), the C4 lesson: a token
  // that does not exist fails silently through the CSS fallback and Stylelint passes either way.
  styles: `
    .page {
    ...
    /*
      A short editable form table, not a data list: the UI Kit's <vf-table> is a scrollable
      PrimeNG datatable with state storage, built for lists rather than for a handful of rows each
      carrying an input. Styled here to the same table language (sticky-free, light separators, no
      zebra) rather than restructured — the deviation is reported, not hidden.
    */
    .vf-table {
      inline-size: 100%;

...
/** Loading state shaped like the table — never a spinner (expiry monitoring ui.md, STD-FE-031). */
@Component({
  selector: 'app-expiry-monitoring-skeleton',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [VfSkeletonComponent],
  template: `
    <div class="skeleton-table" role="status" [attr.aria-label]="t.t('expiry.loading')">
...
export class ExpiryMonitoringSkeletonComponent {
  protected readonly t = inject(TranslationService);
  protected readonly rows = Array.from({ length: 8 }, (_, index) => index);
  protected readonly headerWidths = ['60%', '50%', '45%', '55%'];
  protected readonly cellWidths = ['70%', '50%', '45%', '55%'];
}

...
/// catalog's product code — a counter per branch, allocated inside the transaction that inserts
/// the document (ADR-0022 §6) — not a second numbering scheme. The prefix is the only thing that
/// differs, and it differs on purpose: <c>PRT-</c> is visibly distinct from <c>PUR-</c> so a
/// return can never be misread as an invoice.</para>
/// </summary>

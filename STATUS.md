...
**Updated:** 2026-08-06 · twenty-second cycle · session closed (`/close-session`) · **THE TABLE PRESENTATION STANDARD IS IMPLEMENTED, GATED GREEN, BROWSER-VERIFIED, COMMITTED AND PUSHED TO BOTH `main` AND THE BRANCH — AND THE CLOUD DEPLOY IS DIAGNOSED DOWN TO ONE OWNER ACTION.** **The one owner decision of this session is recorded in TWO places, not one: `design-language.md` §6/§10 (the home the decision lives in) and `docs/business/DECISION_LOG.md` as `BD-DOC-009` (the cross-module index) — the BD-DOC-008 precedent, because the routing rule sends a decision that spans modules to the global log.** **NO OPEN QUESTION IS OUTSTANDING FOR THE OWNER on the presentation work; the single open item is operational and named below (sign in to Render).** *(The wording that follows was written before the commit and is kept as the record of what was verified.)* **THE TABLE PRESENTATION STANDARD IS IMPLEMENTED, GATED GREEN AND BROWSER-VERIFIED.** The owner commissioned a **presentation-only** polish task («No business logic. No API changes. No domain changes.») after seeing two defects on a real screen: numeric values that look detached from their column headers, and a raw machine timestamp in the batch viewer. **⛔ THE LARGEST FINDING IS THAT ONE OF THEM WAS AN APPROVED RULE DEFEATING ITS OWN STATED PURPOSE.** `design-language.md` §6 ruled «محاذاة: النص لليمين **والأرقام لليسار** بأرقام ثابتة العرض **حتى تصطف الخانات**», and the code implemented it exactly (`text-align: end`). **But an Arabic page is RTL while a number is still an LTR run**, so pinning its *end* (left) edge pins the **most significant** digit and lets the units place scatter — the rule's own goal, «حتى تصطف الخانات», was unreachable by the rule's own words. **Measured in headless Chrome rather than argued**, money column 1,234.50 / 5.00 / 87.25: at `end` the digit runs occupied 354–409 · 354–382 · 354–390 (**left edges equal, decimals never aligned, and the header label sat over the currency symbol**); at `start` they occupied 501–556 · 528–556 · 520–556 (**trailing edges identical — الخانات تصطف — and the header edge exactly over the value's**). **THIS COLLIDED WITH AN APPROVED DOCUMENT *AND* WITH A PRIOR OWNER RULING (PIT-002, 2026-08-02: «implement §6 as written»), SO IT WAS STOPPED AND PUT TO THE OWNER RATHER THAN RESOLVED — the contradiction policy, applied.** **OWNER RULED (2026-08-06): align to the inline-start edge and amend §6.** The amendment is in place with **the superseded wording preserved verbatim** (the BR-CAT-020 precedent), it carries the measurement as its evidence, and **it states explicitly that PIT-002's refusal of centred money still stands — only the *edge* moved, centring remains forbidden.** **§10's duplicate sentence («المبالغ محاذاة يسار في عمودها دائمًا») was amended in the same pass**, because leaving the two sections disagreeing is on the ai-governance Never list. **⛔ SECOND FINDING: THE RAW ISO STRING WAS A TYPE MISMATCH, NOT A FORMATTING SLIP.** `BatchViewerItemDto.ReceiveDate` is a **`DateTimeOffset`**, and the screen called `FormatService.date()`, which parses `yyyy-mm-dd` by splitting on `-` — a timestamp made its third part `NaN`, **and the service's documented degrade was to echo the input back**. So `2026-08-02T22:17:31.25352+00:00` reached a clinic's screen. **Reproduced exactly in a before-capture (`2026-08-06T08:53:48.45157+00:00`) before being fixed.** **The amplifier was the degrade, so the degrade is what changed: no formatter can emit machine text any more — every unrenderable value is now `—`.** **`date()` still refuses a timestamp** (deriving a business date from an instant in the browser is BR-INV-059/060's decision, not the client's) — **it simply no longer refuses out loud.** **ONE DEFINITION EACH, IN THE SHARED LAYER, NOT PER SCREEN:** `shared/styles/_numeric.scss` grew from alignment alone to the whole standard (start alignment · tabular figures · nowrap · content-width columns, the width rule stopping at the compact tier where §5 turns tables into cards and a 1% cell would collapse), and **`vf-table` now applies the identical three properties *from the column declaration*, so a screen can no longer half-comply.** `FormatService` gained **`dateOfInstant()`** — literally `dateTimeParts().date`, so those columns **cannot drift** from the two-line stamp — and **`moneyAmount()`** (2 decimals, no symbol) for the one read contract that carries an amount without a currency code. **THREE MORE GAPS FOUND BY AUDIT RATHER THAN BY THE REPORT: (1)** four screens each hand-rolled `createdAt.slice(0, 10)`, which reads the day off the **raw UTC text** and so could name a different day than the stamp beside it; **(2)** the purchase and sale **line-item tables marked only their `<td>`s numeric, never their `<th>`s — so those headers sat on the opposite edge from their own values**, which is the reported symptom in its purest form; **(3)** **both return pages followed no numeric rule at all** — an editable form table drifting away from every list in the product. All closed; the return pages' *entry* column is deliberately left alone (it holds a field, not a figure). **CONSEQUENTIAL DOCUMENTATION, BECAUSE FOUR OTHER DOCUMENTS RESTATED THE OLD EDGE:** `dashboard/ui.md` and `product-editor-ux-architecture.md` state it **for a column** and were amended to follow their source; **`identity/ui.md` and `ui/components.md` state it for a *field* — the phone number — and were NOT changed in behaviour but re-grounded on their real reason** (an LTR run of digits inside an RTL form, `digitsFirst`), each carrying an explicit note that **start-aligning a phone field would break how the number reads**, so the amendment cannot be mis-propagated into it later. The two `pilot-ux-polish-epic*.md` files also name the old rule and were **deliberately left untouched** — they are dated reports of completed work, not live rules. **GATES, ALL GREEN (ADR-0017 §6, in order): `dotnet restore` · backend build 0 warnings / 0 errors · `dotnet format` clean · Domain 163 · Architecture 140 · Integration 292 · ESLint clean · Stylelint clean · frontend 350 (+5) · `ng build` exit 0 (603.49 kB, +0.06 kB — the budget warning is the pre-existing TD-107).** Backend numbers are **unchanged from the previous cycle**, which is the point: nothing outside presentation moved. **Clean diff verified** (no TODO/FIXME/HACK/debug code/untracked files). **LIVE BROWSER VERIFICATION 115/115 at 1440×900 AND 56/56 at 390×844, zero console errors and zero HTTP ≥ 400**, across **fifteen screens** — dev API :5080, dev PostgreSQL :5434, a **fresh `ng serve` on :4500** (:4200 holds stale servers; never verify against it), headless Chrome over CDP. **The harness measures the rendered page rather than reading the source:** for every numeric column on every screen it reports **header-to-value drift 0 px and value spread 0 px**, tabular figures on, a single 2-decimal money precision, and **no ISO text or timezone offset anywhere in any rendered document**. **THE VERIFICATION DATA WAS CREATED THROUGH THE REAL ENDPOINTS, NOT INJECTED:** the signed-in tenant owned **no products at all** (the dev database's older rows sit under a pre-tenancy all-zeros tenant and are invisible to it), so three products, a three-line purchase received with expiries, a committed sale and a draft sale were created **through the actual API** — which is also why the batch viewer could be verified against a **real** receive timestamp. **A before/after capture pair exists** (the working tree was stashed and restored to produce it, and **every gate was re-run afterwards**, because git normalises line endings on write — the recorded CRLF hazard). **ONE TEST ASSERTION WAS CHANGED DELIBERATELY AND IS NAMED RATHER THAN BURIED:** `date() still refuses a timestamp` had pinned the **raw echo**; it now pins that the refusal renders `—` and contains neither `T` nor `+00:00`. **The test's intent — that a business date is never silently derived from an instant — is unchanged and still pinned.** **REPOSITORY STATE: COMMITTED — ONE commit on `pilot/docs-fixes-and-cloud-deployment` carrying all 21 files (15 code · 6 documentation, this file included), by explicit owner approval on 2026-08-06 after the commit gate was run and reported in full. **PUSHED AND VERIFIED** on the owner's word: the push gate (ADR-0017 §7) and the repository-integrity gate (§8) were run first — clean tree · no untracked files · a genuine **fast-forward** (`8207862..0565dcb`, the remote held nothing local did not) · architecture tests green · **all 22 ADR files re-audited against the index by script, zero mismatches** · and the documentation-drift sweep that produced the four consequential amendments. `origin/pilot/docs-fixes-and-cloud-deployment` and local `HEAD` now point at the same commit, confirmed by `git fetch` and a hash comparison rather than by the push command's own output. **`main` WAS THEN FAST-FORWARDED TOO, by a separate explicit owner instruction** («push main too») — deliberately asked for and granted on its own, not folded into the first push, because moving the default branch is its own act. **A genuine fast-forward, verified before the push rather than after: `origin/main` was proved an ancestor of `HEAD` (`git merge-base --is-ancestor`, exit 0) and `HEAD..origin/main` was proved empty, so nothing could be lost and no history was rewritten — `ab28a78..428f885`, no force.** It carried three commits to `main`: `8207862` (the ADR-0010/0021 acceptance, which had been stranded on the branch since the twenty-first cycle), `0565dcb` (this Epic) and `428f885` (the push record). **✅ SO THE LAST STANDING PUBLICATION GAP IS CLOSED: `main` no longer publishes ADR-0010 or ADR-0021 as `Proposed`, and no longer carries the superseded §6.** **Local `HEAD`, `origin/main` and `origin/pilot/docs-fixes-and-cloud-deployment` all point at the same commit; the branch is 0 ahead of `main`** — confirmed by `git fetch` and a three-way hash comparison, not by trusting a push command's output. **The full gate was re-run AFTER the commit as well**, because git normalises line endings on write (the recorded CRLF hazard) and a post-commit run is the only way to know the committed tree is the tree that passed. **⛔ THE GATE ITSELF FOUND THE CYCLE'S LAST DEFECT, IN ITS «SYNCHRONIZED» HALF RATHER THAN ITS AUTOMATED ONE: amending §6 left FOUR other live documents restating the superseded edge**, which would have put contradicting rules in the repository — the Never list. Two state it **for a column** (`dashboard/ui.md`, `product-editor-ux-architecture.md`) and were amended to follow their source; **two state it for a FIELD** (`identity/ui.md`, `ui/components.md` — the phone number, an LTR run of digits inside an RTL form) and were **left unchanged in behaviour but re-grounded on their real reason, each carrying an explicit warning that start-aligning a phone field would break how the number reads** — otherwise a later contributor propagates the amendment into it and calls it consistency. **A stale-server trap was also refused rather than trusted:** port 4500 was still held by a survivor of an earlier `ng serve`, so the confirming browser run was done on a **fresh :4501** instead — 115/115 and 56/56 again, against a provably fresh bundle. **⏳ ALSO IN FLIGHT — THE CLOUD DEPLOY, AND IT IS NO LONGER UNTOUCHED: the owner commissioned «fix the cloud deploy» at the end of this session, and it advanced from *unknown* to *diagnosed and blocked on one owner action*.** **⭐ THE DEPLOYMENT ARTIFACT ITSELF IS PROVED SOUND AT `142d61d`, WITH NO CREDENTIALS AND WITHOUT TOUCHING RENDER OR NEON.** `src/VetFlow.Api/Dockerfile` — **literally what Render builds** — was built locally (**exit 0**, so the build stage is not the failure) and run in **`Production`** against a **throwaway** PostgreSQL on a **throwaway** port (5436) as an **unprivileged role**, deliberately nowhere near the pilot stack on :8080, which was never written to and never restarted. **Results, each closing a step of the nineteenth cycle's five-step resume list: migrations applied at startup as a non-superuser — 23 tables, and `CREATE EXTENSION btree_gin` succeeded (step 2 de-risked)** · **anonymous `GET /` returned `200 text/html`, 13 428 bytes — the `d482424` fix OBSERVED WORKING for the first time, in the published-bundle same-origin mode that `ng serve` STRUCTURALLY CANNOT REACH, which is exactly why that `401` escaped a full browser pass (step 3 de-risked)** · **`GET /api/v1/products` still `401` and an unknown `/api` route still ProblemDetails `404`, so nothing was relaxed to achieve it** · **RLS installed and NOT inert: 16/16 tables `ENABLE` **and** `FORCE`, 16 policies, app role `rolsuper=f`, and a connection with no tenant published saw **0 rows** — fail-closed, proved rather than assumed.** **AND THE PRIME SUSPECT'S EXACT SIGNATURE WAS CAPTURED BY RUNNING THE IMAGE WITH NO KEY: `OptionsValidationException: Jwt:SigningKey must be configured and at least 32 characters.`** — the guard working as designed, and the thing that stopped the Epic's first deploy. **⇒ THE REMAINING FAULT IS CONFIGURATION ON RENDER, NOT CODE.** **The deciding question is whether a `Jwt__SigningKey` env var exists on the service AT ALL: `generateValue: true` is applied when Render CREATES or RE-SYNCS a Blueprint, so a service created before `7dd25db` does NOT gain it from a plain redeploy** — which decides between «Environment → Add → Generate» and a Blueprint re-sync, and is why it must be *looked at* before either is prescribed. Second suspect: the **shape** of `Database__ConnectionString`, which `render.yaml` documents three known ways to get wrong (Neon's `postgresql://` URI instead of Npgsql keyword form · the `-pooler` host while DDL runs at startup · `Trust Server Certificate`, which Npgsql 10 removed). **⛔ BLOCKED ON ONE OWNER ACTION, AND ONLY ONE: RENDER IS NOT SIGNED IN IN THE BROWSER.** The owner approved driving their Chrome read-only, but `dashboard.render.com` redirects to the login screen (GitHub shown as «last used»), and **signing in on the owner's behalf is refused on principle — entering credentials, and completing an SSO handshake that establishes a session and grants OAuth permissions as the owner, are the owner's acts, not the AI's.** **NEXT SESSION STARTS HERE: the owner signs in to Render, then (read-only, changing nothing without asking) — (1) does `Jwt__SigningKey` exist on the service? (2) what is the latest deploy's status and boot log, given `main` was fast-forwarded to `142d61d` this session and a build may have auto-triggered? (3) is `Database__ConnectionString` in Npgsql keyword form on the DIRECT host? Then apply the minimum fix and redeploy.** **⚠️ AND STEP 5 IS NOT A FORMALITY: `select rolsuper from pg_roles where rolname = current_user;` on Neon must be run before this is called green — `true` means the RLS policies are installed and INERT in production, which drops ADR-0022 §8 from four mandatory mitigations to three, and §12.7 declares the shared-database decision of §2 VOID without all four. Steps 1–4 green with step 5 unrun is NOT a green deploy.** **📝 ONE GAP FOUND AND NOT YET CLOSED, BECAUSE CLOSING IT NEEDS THE VALUE: the service URL is recorded NOWHERE in this repository** — `deployment-runbook.md` §Cloud says «Open the Render URL» without saying which, git history has never contained it, and that omission is precisely why this session had to stop and ask. **Recording it in the runbook is part of the fix, not new documentation.** **NOT DONE, DELIBERATELY: `SSL Mode=VerifyFull` — `render.yaml` marks it the hardening step AFTER a deploy is green, so it stays out of scope today.** *(Reproduction, for the next session: `docker build -f src/VetFlow.Api/Dockerfile . -t vetflow-preflight` then run it with `ASPNETCORE_ENVIRONMENT=Production`, `Database__ApplyMigrationsAtStartup=true`, a 32+ character `Jwt__SigningKey`, `Clinic__TimeZone=Africa/Cairo`, and a connection string pointing at a **throwaway** database whose role is `nosuperuser nobypassrls`. Never at the pilot stack.)* **AND A CORRECTION TO THE BLOCK BELOW, WHICH IS NOW PARTLY STALE: `8207862` «docs(adr): accept ADR-0010 and ADR-0021» DID reach the remote branch — `origin/pilot/docs-fixes-and-cloud-deployment` is at it. What is still outstanding is narrower: `origin/main` is ONE commit behind local HEAD**, so `main` alone still publishes ADR-0010/0021 as `Proposed`. **`git push origin HEAD:main` is what closes it.**

**Earlier (twenty-first cycle):** 2026-08-03 · **THE «DASHBOARD» MODULE IS APPROVED, FULLY IMPLEMENTED, VERIFIED AND COMMITTED — EVERY GATE GREEN, BROWSER 22/22.** **A SINGLE commit `9eae4a9`** «feat(dashboard): operational dashboard — لوحة التشغيل» (**79 files, +5 326/−17**) — **one commit for the whole Epic, by owner ruling (2026-08-06)**. *(It had first landed as two — the Epic plus a STATUS correction — and was squashed back to one **before any push**; nothing was public, so no history anyone could have pulled was rewritten.)* **PUSHED AND VERIFIED: `origin/main`, `origin/pilot/docs-fixes-and-cloud-deployment` and local `HEAD` all point at `9eae4a9`; the branch is 0 commits ahead of main.** **`main` WAS FAST-FORWARDED BY EXPLICIT OWNER RULING (2026-08-06: «Push إلى main»), ending the standing choice of the last three cycles** — a genuine fast-forward (`origin/main` was an ancestor: `af3a952..9eae4a9`), **no force, no rewritten history.** That choice had existed for one stated reason: it would publish **ADR-0021 while still `Proposed`**. The consequence was verified and recorded at push time rather than discovered later, **and the owner then closed it the same day: `اعتمد ADR-0010 و ADR-0021` — both are now `Accepted`.** **✅ SO THE OPEN ITEM THE PUSH CREATED IS CLOSED, NOT CARRIED.** **The full commit gate was re-run green immediately before committing AND again after** (git normalises line endings on write — the recorded CRLF hazard — so a post-commit re-run is the only way to know the committed tree is the tree that passed). **The working tree is clean. NOT PUSHED — the push gate is separate and has not been requested.** The owner approved the module and ruled the four open questions (**DEC-DSH-013**): **OQ-DSH-1 — «مخزون منخفض» stays BLOCKED until Catalog introduces a per-product Reorder Level, invent no threshold, build the rest independently** · **OQ-DSH-2 — move the clinic-local «today» to a shared cross-cutting home** · **OQ-DSH-3 — approved; the Arabic name is «لوحة التشغيل»** · **OQ-DSH-4 — keep draft sales.** Plus two more in the same message: **`vf-card` and `vf-stat-tile` approved into the UI Kit**, and **«preserve the architectural rule that the Dashboard is a read-composition module only and must never duplicate business logic or compute domain rules»**. **DOCUMENTATION FIRST, THEN CODE.** **OQ-DSH-2 executed as a MOVE, not a rewrite:** `docs/architecture/cross-cutting/clinic-date.md` is now the definition's home; **BR-INV-059 and BR-INV-060 keep their identifiers and their text**, amended in place to point at it (the BR-CAT-020 / BR-INV-066 precedent). **No rule's meaning changed and no code behaviour changed** — `IClinicClock` was already product-wide infrastructure whose own documentation called itself «the single reference date for every date decision in the system», so this makes the documentation match what the code already did. **THE THREE MODULE-OWNED READS THE COMPOSITION RULE REQUIRES ARE DOCUMENTED AND BUILT, each with identifiers in its OWN module: REQ-INV-013** (BR-INV-070 · AC-INV-066..069 · TS-INV-067..071) · **REQ-SAL-006** (BR-SAL-020 · AC-SAL-023..025 · TS-SAL-030..033) · **REQ-PUR-007** (BR-PUR-019 · AC-PUR-026..027 · TS-PUR-042..044). **THE COMPOSITION RULE IS NOW STRUCTURAL, NOT ADVISORY: `VetFlow.Application.Dashboard` holds a parameterless query and a DTO of primitives and nothing else**, the composition lives in `Infrastructure.Dashboard` (the sanctioned cross-module read path, ADR-0014 §2), and **«Dashboard» was added to the module-isolation architecture test** — so the first import of an Inventory type «to reuse the enum» **breaks the build** (**DEC-DSH-015**). There is not one `Where`, `Count` or comparison in the composer; the movement type crosses as a **string** rather than a re-declared copy of Inventory's closed vocabulary (BR-INV-065). **⛔ THE LARGEST FINDING, AND ONLY THE BROWSER COULD HAVE FOUND IT: THE LANDING ROUTE WAS HARDCODED IN TWO PLACES OUTSIDE THE ROUTE TABLE.** After `app.routes.ts` was changed to `/dashboard`, signing in **still landed on the product list** — the destination was also written by hand in `login-page.component.ts` and in `anonymousOnlyGuard`. **The two unit tests covering those paths were GREEN THE WHOLE TIME, because they asserted the old destination** — a passing suite actively pinning the wrong behaviour. Fixed in all four places (two sources, two tests). **DEC-DSH-016.** **⛔ SECOND BROWSER-ONLY FINDING: the primary action was 39 px tall at 390** — `vf-button` enforced the 44 px floor on the **full-width variant only**. **Fixed in the UI Kit, not on the dashboard**: every button clears 44 px at ≤768 px, desktop untouched. Handling it locally would have been precisely the «one small local exception» §17 warns about, and would have left the identical gap on every other screen. **DEC-DSH-017** — and note the nineteenth cycle found **this exact defect** (a 39 px login action), also in the browser. **THIRD DECISION, WITH ITS TRADE-OFF WRITTEN DOWN: three reads, not seven (DEC-DSH-014)** — one per owning module rather than one per tile. **The cost is recorded rather than hidden: the four inventory sections share a source, so they fail together.** The contract still holds — every section reports its own status and **none is ever rendered as zero** (BR-DSH-014). **BUILT: `GET /api/v1/dashboard`** — one request, seven sections, per-section status, `200` even when a section fails, a failed section **omitted rather than zeroed**, and `clinicDate` returned so the browser never derives a business date of its own. **`/dashboard` is the landing route** (DEC-DSH-011 superseding DEC-IDN-007, propagated to the five identity docs). **The genuinely missing piece was built too: the four destination screens now read their filter from the URL** (`DEC-DSH-006`) — and **no rule was amended and nothing was added to any «حصرًا» list**, because every filter needed already existed in an approved whitelist. **⛔ TWO MORE GAPS FOUND IN SELF-REVIEW, BOTH CLOSED. (4) AN APPROVED RULE PROMISED SOMETHING THE SCREEN DID NOT HAVE:** **BR-DSH-015** and `workflow.md` §4 rule that the board refreshes **«بطلب صريح من المستخدم»**, and `dashboard.refresh` («تحديث») was among the owner's approved nineteen strings — **but no refresh control existed.** The only routes to a reload were the **retry** buttons, which appear **only on failure**, so on a healthy board there was **no way to refresh at all**. A documentation-versus-code contradiction, which `ai-governance.md` lists under **Never**. **A quiet «تحديث» now sits in the header** (quiet — §15.1 allows one primary and it is «بيع جديد»), the approved string is finally used, and **`ui.md` §2 records it**. **(5) THE FAILURE PATH HAD NEVER EXECUTED ANYWHERE:** every test ran against a healthy stack, so all seven sections returned `ok` and the `failed` branch was never reached — in any test **or** in the browser. That is the exact shape of the nineteenth cycle's `401` defect, which survived a full browser pass because its code path only runs under conditions the verification never created. **`DashboardSectionFailureTests` now forces it:** one owning read is replaced with a throwing stub, and the board still returns `200`, the broken section reports `failed`, the other six stay `ok`, and **the failed section carries no `count` key at all**. **Mutation-checked — emitting `Count = 0` from the failed factory turns it red.** *(The first mutation attempt was invalid and was redone: it broke the build, so the suite ran against a stale binary and «passed». The API process was still holding the DLL; it was killed and the check re-run properly.)* **GATES, ALL GREEN: backend build 0/0 · `dotnet format` clean (it caught the recorded CRLF hazard again) · Domain 163 · Architecture 140 (incl. the new Dashboard isolation assertion) · Integration 292 (+18) · ESLint clean · Stylelint clean · frontend 345 (+13) · `ng build` exit 0 (603.43 kB, +2.79 kB; the budget warning is the pre-existing TD-107).** **LIVE BROWSER 22/22 at 1440×900 and 390×844, zero console errors — RE-RUN AND COMPLETE, including the «تحديث» control added in self-review.** Two checks were added for it (the control exists on a **healthy** board, not only on failure · pressing it produces a **second** `/api/v1/dashboard` request in the network log, so it is a genuine re-read and not a page reload). **And the touch-target check was widened in the same pass, which is worth naming: it had been asserting a CHOSEN FEW elements, so the new quiet header actions would have gone unmeasured — the exact class of gap that produced the 39 px defect. It now measures EVERY interactive element on the board (6 at 390 px) and names any that fall short.** The run went against the real stack — dev PostgreSQL :5434, API :5080, and a **fresh dev server on :4500 because :4200 held a stale one** (the trap that produced a bogus report in an earlier cycle). It verified: a signed-out visit cannot reach the board · the landing route · **exactly one `/api/v1/dashboard` request in the network log** · zero-count tiles absent · the draft-purchase tile reading **3 from the real database** · the tile linking to `/purchases?status=draft` · **the destination showing exactly 3 rows with the filter chip visible** · no charts · exactly one money figure · the empty-ledger empty state · 44 px targets. **Population parity was also checked live, not only in tests:** every dashboard count equals its destination screen's `totalCount` for all six filtered links. **TWO STRINGS WERE FLAGGED RATHER THAN PASSED OFF AS APPROVED, AND ARE NOW RULED:** the original nineteen covered neither a per-tile action label (§12 forbids an icon-only affordance) nor the movements empty state (§6 makes one mandatory). Both shipped **marked unapproved in `ar.ts`**, were raised as **OQ-DSH-5**, and the owner ruled them on **2026-08-06** — **«فتح»** and **«لا توجد حركات مسجلة حتى الآن.»**, **in both cases different from what I proposed**. Applied verbatim, flag removed; **twenty-one approved strings, no open question.** **DELIBERATELY NOT BUILT: low stock** (no threshold, no placeholder, no new filter — the owner's ruling) · **no charts** (`vf-stat-tile` has no trend affordance and a test asserts it) · **no auto-refresh** (a test holds the board open and asserts no further request) · **no business action** (a POST to the endpoint is 405). **Full report: `docs/modules/dashboard/implementation-report.md`.** **DEC-DSH-018 records the refresh control** (a quiet header action, because §15.1 allows one primary and it is «بيع جديد»). **REPOSITORY STATE: the Epic is one commit `9eae4a9`, working tree clean, and it IS pushed — `origin/main` and the branch both reached `ab28a78` (Epic + the STATUS record of the push).** **⛔ BUT TWO LATER COMMITS ARE LOCAL-ONLY AND MUST BE PUSHED BY HAND: `ad8e215` «docs(adr): accept ADR-0010 and ADR-0021» (and this STATUS amendment on top of it).** **`git push` HANGS with no output and no error — exit 124 on every attempt, including with `GIT_TERMINAL_PROMPT=0`.** The cause is the **recorded Git Credential Manager block**: `credential.helper = manager` puts up a GUI auth dialog that a non-interactive shell cannot answer. **Reads are unaffected** — `git ls-remote` returns instantly, which is how the failure was distinguished from a network problem. **Two pushes DID succeed earlier in this same session, so the block is intermittent, not permanent.** **THE OWNER MUST RUN: `git push origin HEAD && git push origin HEAD:main`.** Until then **`main` and the branch still show ADR-0010/0021 as `Proposed`, and the ADR-0022 index row still reads `Proposed`** — the fixes exist only locally. **`main` is no longer held back**: the owner ruled it forward on 2026-08-06, ending the standing choice of the last three cycles. **⭐ AND THE SAME DAY THE OWNER ACCEPTED ADR-0010 AND ADR-0021** («اعتمد ADR-0010 و ADR-0021»), so `main` no longer publishes any `Proposed` ADR as shipped architecture. **ADR-0010's own status line had always named its flip condition — «flips to Accepted once the owner reviews this write-up» — and that condition was met.** **ADR-0021 is accepted on a CLOSED risk, not an open one:** it sat `Proposed` because its central accepted risk (shipping to a reachable host **before authentication existed**) was live; Phase 2 has since shipped — authentication, the organization model and tenant isolation are implemented and deployed — and no real clinic data was entered while the gap was open. **A THIRD, STALE ROW WAS FOUND AND FIXED IN PASSING: the ADR index still listed ADR-0022 as `Proposed` while the ADR file itself has said `Accepted` since 2026-08-02** — an index/file contradiction, which the Never list forbids leaving in place. **All 22 ADR files were then audited against the index by script: zero mismatches remain.** **✅ NOTHING IS IN FLIGHT ON THIS EPIC. Every gate has been executed and is green, the browser verification is complete at 22/22, and no work was left half-done.** *(For a future re-run: the harness lives at `…/scratchpad/verify-dashboard.mjs` and needs the dev API on **:5080** — `Database__ConnectionString` pointed at the :5434 dev database, `Clinic__TimeZone=Africa/Cairo` — plus a **fresh** `ng serve --port 4500` (**:4200 holds a stale server; never verify against it**) and headless Chrome on **:9222**.)* **✅ OQ-DSH-5 IS RULED (owner, 2026-08-06) — and the owner chose DIFFERENT wording from what I proposed in both cases, which is the argument for flagging rather than assuming: a proposal does not become approved by being shown.** **`dashboard.tile.open` = «فتح»** (I proposed «عرض»; §12 forbids an icon-only action) and **`dashboard.recentMovements.empty` = «لا توجد حركات مسجلة حتى الآن.»** (I proposed «لا توجد حركات بعد»; §6 makes an empty state mandatory). Both had shipped **marked unapproved in `ar.ts`** and are now applied verbatim, the flag removed. **The module has twenty-one approved strings and NO open question.** `ui.md` §7 and `decisions.md` record the ruling. **⏳ ALSO STILL IN FLIGHT AND UNTOUCHED BY THIS CYCLE: the cloud deploy has never been observed green; its five-step resume list is in the nineteenth-cycle block below.**

**Earlier (twentieth cycle):** 2026-08-02 · **THE «DASHBOARD» MODULE IS COMMISSIONED, DISCOVERED AND DOCUMENTED — PHASE 0 AND PHASE 1 DELIVERED. NO CODE WAS WRITTEN, NOTHING COMMITTED.** The owner commissioned an **Operational Dashboard** — explicitly **not** reporting, **not** analytics, **not** executive — the first screen seen every morning, answering **one** question: «what needs my attention right now?» **The commission was re-issued verbatim after three collisions with Approved artifacts were put to the owner, and that re-issue is what makes this a ruling rather than an inference.** **THE THREE COLLISIONS, EACH HANDLED IN PLACE WITH THE SUPERSEDED WORDING PRESERVED VERBATIM (the BR-CAT-020 / BR-INV-066 precedent): (1) `DEC-IDN-007`** («الشاشة الأولى بعد الدخول = قائمة المنتجات، **ولا لوحة معلومات**») — superseded by **DEC-DSH-011**; **the identifier is neither deleted, renumbered nor reused**, and a «ملاحظة نسخ» section records why it fell (its own stated basis was the inventory scope-lock, and the owner lifted it). **The supersession was propagated to the four other identity docs that state the landing screen** (`overview.md` · `ui.md` · `workflow.md` · `test-scenarios.md`) — each marked forward-looking, because **the Dashboard is Draft and unimplemented, so `catalog/products` is still the truth today**; leaving those four silent would have made the module's docs contradict each other. **TS-IDN-001/016 were NOT rewritten** — they are Approved and pinned to real tests, and they describe reality accurately until the Dashboard ships. **(2) `mvp.md`** gained **§2a**, recording the commission and stating its exact extent: it lifts the dashboard exclusion **and nothing else** from the DEC-SAL-001 scope lock — precisely as DEC-SAL-010 lifted returns alone. **(3) Pilot Observation Mode** prohibits new documentation «unless explicitly requested» — the commission **is** that request; recorded rather than passed over. **PHASE 0 — DISCOVERY IS THE REAL DELIVERABLE, AND ITS OUTPUT IS A REJECTION LIST.** Every candidate was put to the owner's three questions (which business question · which decision · where does the click go). **Eight elements passed; TWELVE WERE REJECTED WITH WRITTEN REASONS** (`decisions.md` §ب): inventory value · today's profit and margins · a weekly sales chart · top-selling products · catalog counters · a welcome card · a notifications centre · branch comparison · depleted batches · separate adjustment/write-off counters · never-received products — **each rejected against a recorded ruling, not against taste** (e.g. the notifications centre dies on **BR-INV-032**, which forbids alerts and background jobs in read screens, and on **DEC-INV-017**, which keeps Monitoring deferred). **⛔ THE ONE ITEM THE OWNER ASKED FOR THAT CANNOT BE BUILT: «LOW STOCK» — BLOCKED, NOT REJECTED.** It fails twice independently: **the number has no definition** (**BR-INV-012** is deferred and **DEC-INV-004** says literally «**لا يُخترع حدّ كمّي عام… لا منطق نائب ولا معيار مؤقت**», the capability being a per-product **Reorder Level owned by Catalog that does not exist**), **and the click-through is also closed** (**BR-INV-014** declares the inventory filter list **«حصرًا»** and contains no low-stock filter — the same rule that forced DEC-INV-040 one month ago). **NO THRESHOLD WAS INVENTED, no «less than 10», no placeholder.** It is **OQ-DSH-1**, and **it blocks that one item and nothing else** — the other seven are independent of it. **PHASE 1 — 9 DOCUMENTS, DOCUMENTATION ONLY:** `docs/modules/dashboard/` with the **standard eight** plus **`api.md`** as a ninth (the `inventory/write-kernel.md` precedent). **`acceptance.md` was written even though the owner's file list omitted it** — **ADR-0017 §5** makes it part of the Definition of Ready, and **a gate does not lapse by omission**. IDs: **REQ-DSH-001..010 · BR-DSH-001..020 · AC-DSH-001..026 · TS-DSH-001..030 · DEC-DSH-001..012**. **THE CENTRAL DECISION — DEC-DSH-001: THE DASHBOARD COMPOSES READS ITS MODULES OWN; IT OWNS NO BUSINESS FACT AND COMPUTES NO NUMBER.** This is not an opinion — it is **DEC-INV-040 applied to six sources instead of one**: that ruling (owner, 2026-08-02) refused to let Catalog sum inventory batches in the browser (duplicating **BR-INV-008** outside its owner) or let the inventory projection gain a filter that violates «حصرًا», and ordered the **owning module to serve its own read**. **THE CONSEQUENCE IS STATED BEFORE APPROVAL RATHER THAN DISCOVERED DURING IMPLEMENTATION: the Dashboard is NOT a self-contained module.** Building it requires **new read requirements amended into `inventory/`, `sales/` and `purchasing/`** — with REQ/BR/AC identifiers **in those modules, not in this one**, each needing its own owner approval (`api.md` §4). **This is written down explicitly because the seventeenth cycle is the cautionary precedent: the S2 stock card was approved on my stated premise that existing endpoints sufficed, and that premise proved false at implementation time.** **THE FINDING THAT MAKES THE MODULE BUILDABLE AT ALL: every filter the navigation contract needs already exists inside an approved whitelist** — «نفاد المخزون» in **BR-INV-014** · «منتهية»/«قرب الانتهاء» in **BR-INV-035** · status in **BR-PUR-004** · status and sale-date range in **BR-SAL-019**. **⇒ NO RULE IS AMENDED AND NOTHING IS ADDED TO ANY «حصرًا» LIST.** What is genuinely missing is smaller and additive: **not one feature list reads `queryParams` today** (verified in code — only `login` does), so filtered deep-linking needs wiring in each destination screen — **DEC-DSH-006**, an implementation dependency with **no business rule involved**. **FOUR MORE DECISIONS WORTH KEEPING: DEC-DSH-002** reconciles the commission's own internal tension — it asked for **one endpoint** *and* **retry states**, which cannot both hold if the response fails as a block; so `GET /api/v1/dashboard` returns **one response with per-section status**, and **⛔ a failed section is NEVER rendered as zero** (BR-DSH-014) — «zero expired batches» and «could not determine expired batches» are contradictory statements, and conflating them is **false reassurance inside a safety decision** (DEC-INV-021). The shape is not invented: the seventeenth cycle already made the S2 card load independently so «a stock outage surfaces a retry in one card instead of blanking a page whose other five loaded». **DEC-DSH-003** was the subtlest problem and the one that changed on review: «today's sales» needs a clinic-local date, but **BR-INV-059 scopes itself by its own text to «مقارنات التواريخ في وحدة المخزون» and enumerates the rules it governs — a Sales figure is in neither.** Saying «the Dashboard adopts BR-INV-059» would have **silently widened an Approved rule's scope**; writing a fresh Dashboard date rule would have created **a second definition of «today»**, which **BR-INV-060's «مصدر واحد لا غير»** exists to prevent. The escape is that `ClinicClock` is **product-wide infrastructure**, tenant-resolved since **DEC-ORG-007**: the Dashboard **reads the same single source, and BR-INV-059's scope is untouched**. Whether that temporal definition should be lifted to a cross-cutting home is **raised, not decided** — **OQ-DSH-2**. **DEC-DSH-012** allows **exactly one money figure** on the whole board (today's sales total) and says why the others fail question (2). **DEC-DSH-010** forbids auto-refresh and polling — **BR-INV-032** bans scheduled refresh and background jobs in read screens, and a board read at the start of a shift is **worse when a stale number looks live**. **TWO RULES ARE MARKED IN THEIR OWN TEXT AS DERIVED FROM AN APPROVED STANDARD RATHER THAN OWNER-RULED** (the C5/C6 idiom): **DEC-DSH-007** (attention items ordered by severity, fixed and not by count — from §4's «الخطر يعلو كلّ شيء حين يظهر») and **DEC-DSH-008** (**one** primary action, «بيع جديد» — three equal buttons would violate §15.1 outright). **A UI KIT EXTENSION IS REQUESTED, NOT INVENTED LOCALLY:** the kit has no card/tile (`badge · banner · button · checkbox · chip · dialog · drawer · empty-state · form-field · input · logo · pagination · popover · select · skeleton · table · theme`), and **§17 forbids a module patching that gap itself** («الانحراف البصريّ يبدأ دائمًا باستثناء صغير محلّيّ لمرّة واحدة») — so **`vf-card` and `vf-stat-tile`** are raised for `components.md`. **The §16 design checklist is answered inside `ui.md` §10, because §16 makes it a condition of approving any UI document.** **FOUR OWNER QUESTIONS, NONE INVENTED (ADR-0017 §5): OQ-DSH-1** Reorder Level — **blocks low stock alone** · **OQ-DSH-2** where the clinic-local-date definition should live — blocks nothing · **OQ-DSH-3** the Arabic name («لوحة التشغيل» proposed, deliberately not «لوحة المعلومات») and the nineteen strings — **blocks implementation, not design** (the OQ-IDN-3 precedent) · **OQ-DSH-4** whether draft-sales stays an attention item — blocks nothing. **REPOSITORY STATE: documentation only.** Nine new documents under `docs/modules/dashboard/` **plus `owner-report.md`** (ten files in the folder), plus in-place amendments to `identity/decisions.md` · `identity/overview.md` · `identity/ui.md` · `identity/workflow.md` · `identity/test-scenarios.md` · `mvp.md` · `docs/modules/_INDEX.md` · `GLOSSARY.md` · this file. **NOT ONE CODE FILE WAS TOUCHED. Nothing committed, nothing pushed.** **NEXT: the owner reviews the Owner Report, approves (or amends) the module and answers OQ-DSH-1..4 — and only then does implementation begin, which will start by seeking approval for the new read requirements in the three owning modules.** **⏳ STILL IN FLIGHT FROM THE NINETEENTH CYCLE AND UNTOUCHED BY THIS ONE: the cloud deploy has never been observed green.** Its five-step resume list is in the block below and remains the first thing to do in any implementation session.

**Earlier (nineteenth cycle):** 2026-08-02 · session closed (`/close-session`) · **THE «SAAS FOUNDATION» EPIC (ADR-0022, Accepted) IS IMPLEMENTED IN FULL, GATED GREEN, BROWSER-VERIFIED, OWNER-APPROVED, AND COMMITTED AND PUSHED** — single commit **`d187dfa`** «feat(saas): SaaS Foundation Epic — multi-tenancy, identity, scope-owned numbering» (**141 files, +15 512/−305**) on `pilot/docs-fixes-and-cloud-deployment`; **local = remote**, the branch is **9 commits ahead of `origin/main`**, and `main` is deliberately **not** fast-forwarded (it would otherwise carry ADR-0021 while it is still `Proposed` — the same condition as the last two cycles). **The full commit gate was re-run green immediately before committing and again after; the tree is clean.** **Phase 0 and the Phase 1/2 backend rode in this commit rather than a separate one** — they had been uncommitted since the previous session, and splitting them would have left a commit where tenancy exists without the isolation that makes it defensible; **the commit message says so rather than hiding it.** **Phases 1–4 all landed.** **PHASE 1 REMAINDER — the clinic clock is now tenant-resolved (DEC-ORG-007, AC-ORG-009):** `ClinicClock` was a singleton resolving **one configured zone at construction**, correct for at most one tenant; it now reads the **tenant's own** zone through a per-tenant cache (`TenantTimeZones`), and **BR-INV-060 is unchanged — only its source moved.** UTC fallback stays prohibited on both ends: `Clinic:TimeZone` still **refuses to boot** when absent or unresolvable (it is now the value a newly seeded clinic starts with, ADR-0022 §10), and an unresolvable **tenant** zone throws rather than guessing. **Proved by two tenants 26 hours apart producing different clinic dates** — a gap no configured-zone implementation could show. **⛔ THE LARGEST FINDING OF THE CYCLE, FOUND BY AUDIT AND NOT BY A TEST: ROW-LEVEL SECURITY DID NOT EXIST.** ADR-0022 §8 makes **four** mitigations mandatory and §12.7 declares the shared-database decision of §2 **void** without all four; three were in place (query filters · the architecture test · composite foreign keys) and **the database-level second net was simply absent** — zero policies, no `app.tenant_id`, nothing. **It is now implemented and, more importantly, it is now EXERCISED.** Policies on **all 16 tenant-scoped tables**, `ENABLE` **and** `FORCE` (an owner is exempt from its own policies unless forced), the predicate **null-safe** so a connection with no tenant published matches **no** rows rather than every row — **fail-closed by construction**. **DEC-ORG-012 records the operational constraint that makes it real: PostgreSQL exempts a `superuser` from RLS unconditionally, so an application running as one would install the second net and never once execute it.** Neon's deployed role is unprivileged — **and the integration suite now creates and runs as an unprivileged role too**, so all 268 tests meet the same policies production does. **This was verified empirically before it was adopted, not assumed.** **RLS immediately earned its keep: it exposed a real latent defect** — two hand-built test contexts wrote `inventory_movements` rows with an **empty tenant id**, invisible to every filtered read and undetectable before the policies existed. **Fixed at the wiring, not at the assertion.** **PHASE 3 — the five database-global sequences are DROPPED.** Numbers now come from a counter row per `(tenant, scope, series)`, allocated with `INSERT … ON CONFLICT DO UPDATE … RETURNING` **inside the transaction that inserts the document** — so the allocator **refuses to run outside one** (structural, not a comment). **Gapless by owner ruling:** proved by a failed create leaving the counter exactly where it was, and by five concurrent creates taking five **consecutive** numbers. **The format is byte-identical** — `PUR-000001` in the browser, same prefixes, same six digits. **A second clinic's first invoice is `PUR-000001`, not `PUR-000002`** (§12.12), proved end to end through a real second tenant that signs in through the real endpoint. `PRD-` is tenant-scoped, `PUR`/`SAL`/`PRT`/`SRT` branch-scoped. **PHASE 2 — the login screen and its plumbing.** `/login` outside the shell (there is no navigation to offer someone who is not signed in), a `CanMatch` guard so an unauthenticated visit never even downloads a feature bundle, an HTTP interceptor attaching the bearer token and turning a refused one into a **visible** return to the login screen, and sign-out at the foot of the sidebar. **The nine approved Arabic strings are used verbatim and no tenth was invented.** **The landing route is unchanged — `catalog/products`, no dashboard built (DEC-IDN-007).** «انتهت جلستك» is distinguished **on the client** by whether a token was actually held (DEC-IDN-016) — a request sent with no token was never a session, and is not told one ended; **no second server contract was invented for it.** The token lives in `localStorage` (DEC-IDN-015) because BR-IDN-009 allows a session to end **only** by explicit sign-out or expiry, and an in-memory token would end it by a **third** route: a page refresh. **⛔ SECOND FINDING, ALSO UNIMPLEMENTED: `ICurrentUser` WAS REGISTERED AND NEVER CONSUMED** — so **BR-INV-066 as amended** (every operation belongs to an authenticated performer, REQ-IDN-008 / AC-IDN-011) did not exist. **Now implemented in the DEC-ORG-011 shape**: a shadow `PerformedByUserId` stamped from the token's claims by an interceptor, so **no call site carries it and none can forget it**, and an **unauthenticated write is refused rather than attributed to nobody**. **`ActorName` is untouched** — historical values stay readable, it is simply no longer the source. Pinned by an architecture test **and** by a test proving a client-supplied `actorName` changes attribution not at all. **⛔ THIRD FINDING, CAUGHT IN SELF-REVIEW: ADR-0022 §5's LAST SENTENCE WAS UNMET** — «the GIN trigram indexes must be rescoped alongside their unique B-tree counterparts». The unique indexes had been rescoped; **the six tenant-scoped GIN indexes had not** (categories · manufacturers · products ×2 · purchase invoices · sales invoices). **Not a leak** — the filter and RLS both still apply — **but an explicit «must» in an Accepted ADR, and it was about to be reported as satisfied.** Now rescoped to lead with the tenant, which needs the **`btree_gin`** extension to carry a `uuid` inside a GIN index; **it is a trusted extension and the unprivileged role created it successfully in the verification database before it was adopted** (the `pg_trgm` precedent). **`product_natures` is deliberately left alone** — platform-global, no tenant to lead with. Recorded as **DEC-ORG-016**. **GATES, ALL GREEN: backend build 0 warnings / 0 errors · `dotnet format` clean · Domain 163 · Architecture 140 (+1) · Integration 268 (+25) · frontend ESLint clean · Stylelint clean · 332 unit tests (+17) · `ng build` exit 0 (600.64 kB, +4.60 kB over the branding cycle — TD-107's budget warning is pre-existing and unchanged in kind).** **TRACEABILITY: the 29 approved scenarios of the two new modules are now named in test method names — TS-IDN-001..015 and TS-ORG-001..013 map onto real tests; TS-IDN-016 is the browser script, which is a run and not a named test.** **LIVE BROWSER VERIFICATION 45/45 at 1440×900 AND 390×844, zero console errors** — headless Chrome over CDP against the real API and a real PostgreSQL **running as the unprivileged role**: the guard, the three validation moments, the single failure message, the banner taking focus and clearing on edit, a submit button never disabled for invalidity, the ruled landing screen, sign-out, a **refused token** producing `/login?expired=1` with the approved sentence, the ten existing screens as regression, and the 44 px touch target. **It found one real gap the unit tests could not: the login action was 39 px tall on the phone** — the compact tier's own navigation holds itself to 44, so the full-width action now does too. **The run also re-checks, deliberately, the exact defect the seventeenth cycle found in the browser and could never find in jsdom (which implements neither `visibility` nor `inert` for focus): opening the drawer at 390 must move focus INTO it. It does — and it matters more now than then, because the shell moved from the application root into a routed layout, so its focus deferral runs on a router-driven mount.** **Performance: p50 5–10 ms across five endpoints under RLS; the extra `set_config` per connection open is not measurable at clinic volume.** **FIVE IMPLEMENTATION-TIME DECISIONS RECORDED IN THE ESTABLISHED «اكتُشف وقت التنفيذ» IDIOM rather than deviating silently: DEC-ORG-016** (the GIN indexes lead with the tenant, and `btree_gin` is what makes that expressible) · **DEC-ORG-012** (RLS needs `FORCE` and a non-superuser role) · **DEC-ORG-013** (the tenant is published per connection open and cleared when absent — pooled connections would otherwise hand one tenant's scope to the next borrower) · **DEC-ORG-014** (one `scope_id` column instead of a nullable branch column — a nullable member of a primary key is not expressible in PostgreSQL; and `ON CONFLICT` is `UPDATE … RETURNING` plus first-use creation, so a new branch needs no provisioning step) · **DEC-ORG-015** (the tenant time zone is cached for the process; changing it directly in the data needs a restart — accepted because no screen can change it) · plus **DEC-IDN-015/016** for the client session. **UI Kit: `vf-text-input` gained `password`/`tel`, `inputMode`, `autocomplete` and `digitsFirst`; `vf-button` gained `full` — all additive, recorded in `components.md`.** **NOT DONE, DELIBERATELY AND NAMED: attribution was implemented on the movement ledger only** — that is where BR-INV-066 rules it and where a recorded attribution concept exists; a `created_by` on all seventeen tables was **not** invented. **FIRST CLOUD DEPLOY OF THE EPIC — TWO FAILURES, BOTH FIXED, ONE OF THEM A REAL DEFECT.** **(1) `Jwt:SigningKey must be configured and at least 32 characters` — the guard working as designed** (a predictable signing key would let anyone mint a token for any tenant), **but `render.yaml` never declared the variable**, so the service had nowhere to hold one. Declared with **`generateValue: true`** in **`7dd25db`**, so Render creates the secret itself and it never reaches the repository or a transcript — which matters, because a live credential in a transcript is already an open item from the sixteenth cycle. **(2) A GENUINE DEFECT THE EPIC INTRODUCED: the deployed app answered `/` with `401 VTF-IDN-002`, so the login screen could not load and nobody could sign in to find out why.** Two causes, both from the new fallback authorization policy: **static files were served after `UseAuthorization`** — that middleware applies the fallback policy to requests that matched **no endpoint at all**, which is what a bundle file is — **and the SPA fallback endpoint had no exemption**, routing matching it before the static-file middleware (which steps aside whenever an endpoint is already matched), so the index document was refused on its own account. **Fixed in `d482424`: static files move ahead of authorization, and the fallback endpoint is `AllowAnonymous`. Nothing else is relaxed** — REQ-IDN-006 names this exact exception («the sign-in endpoint, and what the interface needs in order to be shown before signing in»), every business endpoint still demands a token, and an unknown `/api` route is still a ProblemDetails 404 rather than the shell. **WHY IT ESCAPED: the same-origin hosting block executes only when a published bundle is present, and the browser verification ran against `ng serve`, where it never does.** **`SpaHostingTests` now boots the host with a real web root and covers that mode — six tests, mutation-checked: removing either half of the fix fails them.** Integration **274** (+6). **Recorded as DEC-IDN-017**, because a future contributor tightening the anonymous surface would break sign-in again and the reason must not live only in a commit message. **⏳ IN FLIGHT AT SESSION CLOSE — THE CLOUD DEPLOY, AND NOTHING ELSE.** Four commits are on the branch and the working tree is clean: **`d187dfa`** (the Epic) · **`7dd25db`** (blueprint) · **`d482424`** (the shell fix + `SpaHostingTests`) · **`be82688`/`1a8f2b7`** (STATUS). **The deploy has not been observed green: the last log seen was the `401` failure, and the fix went out after it.** **NEXT SESSION STARTS HERE, in order: (1) set `Jwt__SigningKey` on the Render service (dashboard → Environment → Generate — never pasted into a transcript) and redeploy this branch; (2) read the migration log for `CREATE EXTENSION btree_gin` — it is the one statement in five new migrations that could fail on Neon, and it is trusted so it should not; (3) open the site anonymously and confirm the login screen renders (that is the `d482424` fix landing in production, and it has never been seen there); (4) sign in as `01001127204`/`01001127204` and walk one real screen; (5) run `select rolsuper from pg_roles where rolname = current_user;` on Neon — `true` means the row-level-security policies are installed and INERT in production, leaving only the EF filter, and that is a DEC-ORG-012 stop-and-fix, not a note.** **Nothing else is in flight. No work was left half-done, and no gate is red.**
